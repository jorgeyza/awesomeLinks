import { useMultiStyleConfig, type SystemStyleObject, type ThemingProps } from '@chakra-ui/react';
import { type ImageConfigComplete } from 'next/dist/server/image-config';
import { ImageLoaderProps, type ImageProps as NextImageProps } from 'next/image';
...
export type GenerateImageAttributesReturn = Pick<HTMLImageElement, 'src'> & Partial<Pick<HTMLImageElement, 'sizes' | 'srcset'>>;
...
export const defaultLoader = ({ src, width, quality = defaultQuality }: ImageLoaderProps): string =>
  `${configPath}?url=${encodeURIComponent(src)}&w=${width}&q=${quality}`;
...
export const useImageAttributes = ({
  ...
  useMemo(() => {
    ...
    if (src && (src.startsWith('data:') || src.startsWith('blob:'))) {
      ...
      imgAttributes = { sizes: undefined, src, srcset: undefined };
      ...
      const widths = [
        ...new Set(
           ...
           */
          [width, width * 2].map((w) => configAllSizes.find((s) => s >= w) || configAllSizes[configAllSizes.length - 1])
        )
      ];
      imgAttributes = {
        sizes: undefined,
        src: loader({ quality, src, width: widths[1] }),
        srcset: widths.map((w, i) => `${loader({ quality, src, width: w })} ${i + 1}x`).join(', ')
      };
    } else {
      ...
      imgAttributes = {
        sizes: widths.map((w, i) => (i < widths.length - 1 ? ` (max-width: ${w}px) ${w}px` : ` ${w}px`)).join(','),
        src: loader({ quality, src, width: widths[widths.length - 1] }),
        srcset: widths.map((w) => `${loader({ quality, src, width: w })} ${w}w`).join(', ')
      };
    }
...
export const useImageStyle = ({
  ...
  const { layPicture, layPictureCls, layImage, layImageCls, preImage, preImageBlur, preImageCls } = useMultiStyleConfig('Image', { variant });
  ...
  const withBlurPlaceholder = !!(
    isMounted && // wait for the first render to add blur, in case JS is disabled
    !imgLoaded &&
    blurDataURL &&
    (!height || height > 48) &&
    (!width || width > 48) &&
      ...
      image: {
        ...
        ...(withBlurPlaceholder
          ? {
              ...
              backgroundPosition: 'var(--blurBackgroundPosition)',
